...
export default function Signup() {
    ...
    const handleSignup = async () => {
        ...
        // Validate form fields
        if (!formData.email || !formData.password || !formData.confirmPassword || !formData.website_id) {
            setErrors({
                email: !formData.email ? 'Email is required' : '',
                password: !formData.password ? 'Password is required' : '',
                confirmPassword: !formData.confirmPassword ? 'Confirm Password is required' : '',
                website_id: !formData.website_id ? 'Website ID is required' : '',
            });
            return;
        }
        if (!validateEmail(formData.email)) {
        ...
        // Fetch from nodeee
        try {
            const response = await fetch('http://127.0.0.1:8000/crispchat/register/', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                        ...
                        <div className={styles.inputs}>
                            ...
                            <div className={styles.fields}>
                                ...
                                <div className={styles.inputContainer}>
                                    ...
                                    />
                                    <div className={styles.error}>{errors.website_id}</div>
                                </div>
                            </div>
                        </div>
                        <div className={styles.login_button} onClick={handleSignup}>
